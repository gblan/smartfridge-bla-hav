...
public class DetailMenuController {
	...
	public DetailMenuController(FridgeManager fridge) {
		...
		this.rightControl.getView().getProductList()
				.addMouseListener(productListener);
		this.leftControl.getView().getDecreaseQuantityButton()
				.addActionListener(buttonLess);
		this.leftControl.getView().getIncreaseQuantityButton()
				.addActionListener(buttonMore);
	}
	...
	private ActionListener buttonLess = new ActionListener() {
		...
		public void actionPerformed(ActionEvent e) {
			if ((leftControl.getView().getActualQUantity().getText()
					.matches("\\d+"))) {
				fridge.executeAction(TypeActionEnum.DECREASE_QUANTITY,
						rightControl.getSelectedProduct(), 1);
	...
	private ActionListener buttonMore = new ActionListener() {
		...
		public void actionPerformed(ActionEvent e) {
			if ((leftControl.getView().getActualQUantity().getText()
					.matches("\\d+"))) {
				fridge.executeAction(TypeActionEnum.INCREASE_QUANTITY,
						rightControl.getSelectedProduct(), 1);
				leftControl.refreshData(rightControl.getSelectedProduct());
			}
	...
	private MouseListener productListener = new MouseListener() {
		...
		public void mouseClicked(MouseEvent e) {
			if (e.getClickCount() == 2) {
				leftControl.refreshData(rightControl.getSelectedProduct());
				rightControl.refreshSelected(rightControl
