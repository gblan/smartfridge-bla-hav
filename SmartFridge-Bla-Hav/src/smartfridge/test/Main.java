...
public class Main {
	...
	private static void add(FridgeManager fm) {
		...
		System.out.println();
		System.out
				.println("To add a new product, follow this format and press enter :");
		System.out
				.println("Type#;Name ;Expiration(dd/MM/yyyy)(optional);Quantity(optional)");
		fm.clearRedoList();
		try {
			...
			if (line[2].equalsIgnoreCase(" ")) {
				if (line[3].equalsIgnoreCase(" ")) {
					fm.executeAction(
							TypeActionEnum.ADD,
							new ProductUnPerishable(
									TypeProductEnum.values()[Integer
											.parseInt(line[0])], line[1], 1), 0);
				} else {
					fm.executeAction(
							TypeActionEnum.ADD,
							new ProductUnPerishable(
				...
				if (line[3] == " ") {
					fm.executeAction(
							TypeActionEnum.ADD,
							new ProductPerishable(
									TypeProductEnum.values()[Integer
											.parseInt(line[0])], line[1], 1,
									calendar), 0);
				} else {
					fm.executeAction(
							TypeActionEnum.ADD,
							new ProductPerishable(
									TypeProductEnum.values()[Integer
											.parseInt(line[0])], line[1],
									Integer.parseInt(line[3]), calendar), 0);
	...
	private static void remove(FridgeManager fm) {
		...
		String line;
		System.out
				.println("Select the ID of the product you want to remove : ");
		fm.clearRedoList();
		try {
			line = console.readLine();
			int id = Integer.parseInt(line);
			fm.executeAction(TypeActionEnum.REMOVE, fm.getFridge()
					.getFridgeContent().get(id - 1), 0);
		} catch (IOException e) {
			System.err.println("Id Invalide");
		}
	}
	...
	private static void increaseQuantity(FridgeManager fm) {
		...
		String line;
		System.out
				.println("Select the ID of the product you want to increase the quantity : ");
		fm.clearRedoList();
		try {
			line = console.readLine();
			int id = Integer.parseInt(line);
			fm.executeAction(TypeActionEnum.INCREASE_QUANTITY, fm.getFridge()
					.getFridgeContent().get(id - 1), 1);
		} catch (IOException e) {
			line = "Reading error";
			e.printStackTrace();
		}
	...
	private static void decreaseQuantity(FridgeManager fm) {
		...
		try {
			...
			fm.executeAction(TypeActionEnum.DECREASE_QUANTITY, fm.getFridge()
					.getFridgeContent().get(id - 1), 1);
		} catch (IOException e) {
			line = "Reading error";
			e.printStackTrace();
	...
	private static void checkExpiration(FridgeManager fm) {
		...
		String line;
		System.out
				.println("Select the number of days to check the expiration date");
		try {
			line = console.readLine();
		...
		while (!exit) {
			...
			try {
				...
				case "sda":
					...
					break; // done
				case "sed":
					fm.sortProductByPerishmentDate();
					break; // done
				case "siq":
					fm.sortProductByQuantity(true);
					break; // done
				case "sqd":
					fm.sortProductByQuantity(false);
					break; // done
				case "ud":
					fm.unDoAction();
					break; // done
				case "rd":
					...
					break; // done
				case "sa":
					FridgeUtils.saveFridge(fm.getFridge());
					break; // done
				case "lo":
					fm.setFridge(FridgeUtils.loadFridge());
					break; // done
				case "ex":
					exit = true;
					break;// ça marche
